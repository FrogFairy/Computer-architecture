...
Axis::Axis(Orientation d, Point xy, int length, int n, const std::string& lab) : label{Point{0, 0}, lab}
{
  if (length < 0)
...
Marked_polyline::Marked_polyline(const std::string& m, std::initializer_list<Point> lst) : Open_polyline{lst}, mark{m}
{
  if (m == "")
    mark = "*";
}
